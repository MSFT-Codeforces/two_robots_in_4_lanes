...
**Note:-**
- In the first example, test case 1 has only one tier "...." (no artifacts), so both robots can stay at their starting corridors $(1,4)$ and the total energy is $0$.

- In the first example, test case 2 has one tier "####", so $|S|=4 \ge 3$ and it is impossible to handle all artifacts with two robots; the answer is $-1$.

- In the first example, test case 3 is processed bottom-to-top: first "...." (no constraint), then "#..." (artifact in corridor $1$). Keeping $(A,B)=(1,4)$ throughout satisfies the last tier (robot $A$ is already in corridor $1$), so the total energy is $0$.

- In the first example, test case 4 is processed bottom-to-top: first "#..#" forces the robots to be in corridors $\{1,4\}$, which matches the start $(1,4)$ with cost $0$. Then ".##." forces corridors $\{2,3\}$; choosing $(A,B)=(2,3)$ costs
  $$
  |2-1|+|3-4|=2,
  $$
